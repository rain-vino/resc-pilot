...
* - Remove getting param process of fixed parameters.
*
...
void SDFMap::initMap(const ros::NodeHandle& nh) {
    node_ = nh;

    /* get parameter */
    double x_size, y_size, z_size;
    mp_.resolution_ = 0.1;
    node_.param("map/map_size_x", x_size, 10.0);
    node_.param("map/map_size_x", y_size, 10.0);
    node_.param("map/map_size_x", z_size, 3.0);

    mp_.local_update_range_.x() = 7.5;
    mp_.local_update_range_.y() = 7.5;
    mp_.local_update_range_.z() = 2.0;

    mp_.obstacles_inflation_ = 0.1;

    // depth camera intrinsics
    // Intel RealSense D435i, 848x480, /camera/depth/image_rect_raw
    // roslaunch realsense2_camera rs_camera.launch (align_depth:=false)
    depth_topic_ = "/camera/depth/image_rect_raw";
    mp_.fx_ = 425.9174;
    mp_.fy_ = 425.9174;
    mp_.cx_ = 432.9769;
    mp_.cy_ = 238.5758;

    // Intel RealSense D435i, 640x480, /camera/aligned_depth_to_color/image_raw
    ...
    mp_.esdf_slice_height_ = 0.3;
    mp_.visualization_truncate_height_ = 1.6;
    ...
    mp_.local_map_margin_ = 50;
    mp_.ground_height_ = 0.0;
    ...
    mp_.prob_hit_log_ = m_logit(mp_.p_hit_);
    mp_.prob_miss_log_ = m_logit(mp_.p_miss_);
    mp_.clamp_min_log_ = m_logit(mp_.p_min_);
    mp_.clamp_max_log_ = m_logit(mp_.p_max_);
    mp_.min_occupancy_log_ = m_logit(mp_.p_occ_);
    ...
    /* init callback */
    depth_sub_ = std::make_shared<message_filters::Subscriber<sensor_msgs::Image>>(node_, depth_topic_, 50);

    if (mp_.pose_type_ == POSE_STAMPED) {
        std::cout << "pose type: pose stamped" << std::endl;
        pose_sub_ = std::make_shared<message_filters::Subscriber<geometry_msgs::PoseStamped>>(node_, pose_topic_, 25);
        ...
        odom_sub_ = std::make_shared<message_filters::Subscriber<nav_msgs::Odometry>>(node_, "/sdf_map/odom", 100);
    ...
    std::cout << "Fast-Planner SDF Map initialized." << std::endl;
}

void SDFMap::resetBuffer() {
    const Eigen::Vector3d min_pos = mp_.map_min_boundary_;
    const Eigen::Vector3d max_pos = mp_.map_max_boundary_;
...
void SDFMap::updateESDF3d() {
    ...
    for (int x = min_esdf[0]; x <= max_esdf[0]; x++) {
        for (int y = min_esdf[1]; y <= max_esdf[1]; y++) {
            fillESDF(
                    ...
                    [&](int z, const double val) { md_.tmp_buffer1_[toAddress(x, y, z)] = val; }, min_esdf[2],
                    max_esdf[2], 2);
        }
    }
    ...
    for (int x = min_esdf[0]; x <= max_esdf[0]; x++) {
        for (int z = min_esdf[2]; z <= max_esdf[2]; z++) {
            fillESDF([&](int y) { return md_.tmp_buffer1_[toAddress(x, y, z)]; },
                     [&](int y, const double val) { md_.tmp_buffer2_[toAddress(x, y, z)] = val; }, min_esdf[1],
                     max_esdf[1], 1);
        }
    }
...
int SDFMap::setCacheOccupancy(Eigen::Vector3d pos, int occ) {
    ...
    Eigen::Vector3i id;
    posToIndex(pos, id);
    const int idx_ctns = toAddress(id);
...
void SDFMap::projectDepthImage() {
    ...
    const Eigen::Matrix3d depth_optical_to_camera = 
    (Eigen::Matrix3d() << 0, 0, 1,   // z_optical -> x_camera
                          -1, 0, 0,    // -x_optical -> y_camera  
                          0, -1, 0    // -y_optical -> z_camera
    ).finished();
    const Eigen::Matrix3d camera_to_world = md_.camera_q_.toRotationMatrix();

    Eigen::Matrix3d camera_r = depth_optical_to_camera * camera_to_world;
